...
import { selectMessages, toggleChat, selectIsChatOpen, selectIsConnected } from '../features/chat/chatSlice';
import { 
...
const ChatPanel = () => {
  ...
  const [sendMessage, { isLoading: isSending, isError: sendError }] = useSendMessageMutation();
  const [connect, { isLoading: isConnecting }] = useConnectToChatMutation();
  const [disconnect] = useDisconnectFromChatMutation();
  
  const messageEndRef = useRef<HTMLDivElement>(null);
  
  // Connect to chat when panel is opened
  useEffect(() => {
    if (isOpen && !isConnected && !isConnecting) {
      connect().unwrap().catch((error) => {
        toast({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      await sendMessage(newMessage.trim()).unwrap();
      setNewMessage('');
    } catch (error) {
